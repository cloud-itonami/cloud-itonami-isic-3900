(ns remediationops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (RemediationOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SiteRemediationOpsGovernor
  ...
  human operator/remediation-project supervisor. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [remediationops.advisor :as advisor]
            [remediationops.governor :as governor]
...
(defn build
  ...
  `remediationops.store/Store`).
  opts:
    :advisor      -- a `remediationops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RemediationOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SiteRemediationOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
